...
## Known state (test course SSY300)

- **Python extraction: done** — 48/48 materials.
- **AI extraction + comparison/evaluation: INVALID, must be re-run.** They were
  produced while `material-extraction` was on an agent provider and
  `extraction-validation` on a text-only Ollama model — both blind to images.
- **Routing is now fixed** — both tasks point at **TOK-Gemini / `gemini-2.5-flash`**
  (vision, token type). Verified responding.
- **Recovery still pending:** re-run AI extraction with `overwrite=true` on the
  affected files, then re-Compare and re-Evaluate. Nothing is promoted to
  `final/` yet. Use **Verify** to reconcile any stale statuses with disk.
