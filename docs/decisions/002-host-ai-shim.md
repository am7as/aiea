...
The shim binds to `127.0.0.1` only. It has no authentication, it takes `working_dir` from
the request body, and at the `full` tier it runs the CLI with permissive flags. Binding it
to every interface would hand arbitrary local code execution, as the host user, to anyone
on the same network. It was bound to `0.0.0.0` until 2026-08-15; if you have an older copy,
change `server.listen(PORT, …)` in `scripts/host-ai-shim.mjs`.

Docker containers still reach it at `host.docker.internal:4023` on Docker Desktop, because
that address is proxied by the host rather than routed from the VM's own interface.
