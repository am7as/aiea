...
Open `http://localhost:4022`. No login. You are in.

That is Caddy, the single entry point. Port 4020 reaches the frontend directly and is
useful when you are debugging, but 4022 is the one to use.
...
4. **Rescan + ingest.** In Workshop's Materials panel, click **🟦 Rescan materials**. AIEA walks the folder, registers each file as a `Material` row in the database, and shows it as `pending`. Then either:
   ...
   Extracted text lands in `<workshop>/extracted/py/<material-id>/extracted.md` (the AI channel writes to `ai/`, and the version you pick lands in `final/`) — Obsidian-friendly Markdown with page-aware sections.

5. Generate questions, refine them, assemble an exam, and export to LaTeX using your
   `exam-template/`. See [guide 05](guide/05-generate-refine-promote.md) and
   [guide 07](guide/07-building-exams.md).

## Connecting AI providers

See [guide 06](guide/06-ai-engine.md) for the walkthrough and
[docs/ai-engine.md](ai-engine.md) for the reference. Subscription providers reach your
logged-in CLI through the host shim, not through `creds/`.
