...
Wait for `Container aiea-api Started`. The first build takes ~3–5 min; later runs are seconds.
...
- Click the **Browse** button to pick a parent folder visually. AIEA's folder picker is sandboxed to `~/Downloads`, `~/aiea`, `~/Documents` by default — see [02 - Connecting your folders](02-connecting-folders.md) to add more.
