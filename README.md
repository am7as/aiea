...
Four tasks need a model that can read images: `material-extraction`,
`extraction-validation`, `answer-validation` and `figure-refinement`. Route those to a
vision-capable provider. An agent-mode provider can never do them, because it sends text
only, and a text-only model fails silently, which is worse than failing loudly.
...
AIEA has no login. It is safe only because nothing it opens is reachable from your network.

- **Every port is bound to `127.0.0.1`.** If you change the ports in
  `infra/docker-compose.yml`, keep that prefix. Without it, the unauthenticated API and the
  Postgres instance (which uses the default `aiea` / `aiea` credentials) are exposed to
  everyone on your network.
- **The host AI shim is loopback-only for the same reason.** It runs your logged-in CLIs
  with permissive flags, so anything that can reach it can run commands as you.
- **Provider API keys are stored in plain text** in the `providers` table in Postgres. They
  are masked when the browser reads them back, but anyone with access to the database file
  or a `pg_dump` can read them. Encrypting them at rest is not done yet.
- **Do not put AIEA on a shared or public host** without putting authentication in front of
  it.
